<template>
  <div class="row">
    <div v-for="(question, index) in questions" :key="index" class="col-md-12">
      ...
      <div class="card read-only-question cursor_grab" v-if="selectedQuestion.id !== question.id">
        <div class="">
          ...
          <div class="p-0 text-right mt-3">
            <a href="#" class="mr-3 text-link custom-text-link" @click.prevent="editQuestion(question, index)">Edit</a>
            <a href="#" class="text-link custom-text-link color-red" v-on:click="deleteQuestion(question, index)">Delete</a>
          </div>
        </div>
      </div>
    </div>
...
.branching_section .branching_image {
    max-width: 120px;
    max-height: 120px;
    width: 100%;
